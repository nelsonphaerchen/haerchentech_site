...
var require_front_matter = __commonJS({
  "node_modules/front-matter/index.js"(exports, module2) {
    ...
    function computeLocation(match2, body) {
      var line = 1;
      var pos = body.indexOf("\n");
      var offset = match2.index + match2[0].length;
      while (pos !== -1) {
        if (pos >= offset) {
          return line;
        }
    ...
    function parse(string, allowUnsafe) {
      ...
      var line = computeLocation(match2, string);
      return {
        attributes,
        body,
        bodyBegin: line,
...
var serveStatic = (options2) => {
  ...
  const join = options2.join ?? defaultJoin;
  return async (c, next) => {
    if (c.finalized) {
      return next();
    }
    ...
    if (options2.isDir && await options2.isDir(path)) {
    ...
    if (content != null) {
      const mimeType = options2.mimes && getMimeType(path, options2.mimes) || getMimeType(path);
...
var getContentFromKVAsset = async (path, options2) => {
  let ASSET_MANIFEST;
  if (options2 && options2.manifest) {
    if (typeof options2.manifest === "string") {
      ASSET_MANIFEST = JSON.parse(options2.manifest);
    } else {
      ASSET_MANIFEST = options2.manifest;
    }
  } else {
  ...
  let ASSET_NAMESPACE;
  if (options2 && options2.namespace) {
    ASSET_NAMESPACE = options2.namespace;
  } else {
  ...
  return async function serveStatic22(c, next) {
    const getContent = async (path) => {
      return getContentFromKVAsset(path, {
        ...
        // @ts-ignore
        namespace: options2.namespace ? options2.namespace : c.env ? c.env.__STATIC_CONTENT : void 0
      });
    };
    return serveStatic({
      ...options2,
  ...
  emStrong(src, maskedSrc, prevChar = "") {
    ...
    if (!nextChar || !prevChar || this.rules.inline.punctuation.exec(prevChar)) {
      ...
      while ((match2 = endReg.exec(maskedSrc)) != null) {
        ...
        const raw2 = src.slice(0, lLength + match2.index + lastCharLength + rLength);
...
app.use("/images/*", module({ root: "./posts/" }));
function getPosts() {
  const posts = [];
  for (const path in __exports) {
    const slug = path.split("/").pop().replace(".md", "");
    const content = __exports[path];
    const parsed = (0, import_front_matter.default)(content);
    posts.push({
      slug,
      title: parsed.attributes.title || slug,
      date: parsed.attributes.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
...
app.get("/", (c) => {
  ...
  `;
  return c.html(renderLayout("Terminal Blog", html2));
});
app.get("/post/:slug", (c) => {
  const slug = c.req.param("slug");
  const postPath = `../posts/${slug}.md`;
  const rawContent = __exports[postPath];
  if (!rawContent) {
    return c.text("Post n\xE3o encontrado", 404);
  }
  const parsed = (0, import_front_matter.default)(rawContent);
  ...
  `;
  return c.html(renderLayout(parsed.attributes.title, html2));
});
app.get("/portfolio", (c) => {
  const html2 = `
    <h1>Meus Projetos</h1>
    <ul>
        <li><strong>Projeto 1</strong>: Automa\xE7\xE3o em Node.js <br><a href="https://github.com" target="_blank">[Ver no GitHub]</a></li>
    </ul>
  `;
  return c.html(renderLayout("Portf\xF3lio", html2));
});
app.get("/contato", (c) => {
  const html2 = `
    <h1>Contato</h1>
  ...
  `;
  return c.html(renderLayout("Contato", html2));
});
